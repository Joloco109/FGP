...
def leer(am):
    ...
    empty = Hist.read( directory + leer_names[ 1 if am else 0 ], "Leer", "Leermessung Americium" if am else "Leermessung Roehre")
    if am:
    ...
    for i in range(len(leer_peaks)):
        empty_peaks = Hist.read( directory + leer_peaks[i] , "Leer", "Leermessung Peaks Americium" if am else "Leermessung Peaks Roehre") 
        ...
        for peak in peaks:
            offset = offset_am if am else offset_r
            print(peak)
            ...
            for j in range(0, int((peak.GetNpar()-1)/3) ):
                ...
                print("mu_{}  = {:=8.3f} +- {:=8.3f}".format( j, peak.GetParameter(3*j+2)+offset[i], peak.GetParError(3*j+2) ))
