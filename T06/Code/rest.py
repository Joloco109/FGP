...
files = [ [ "Kupfer_20V.mca", "Kupfer_35V.mca", "Kupfer.mca", ],
    [ "Blei.mca", "Pickup_rejection.mca"]
    ]
...
def identify_file( file_number, am, cali=None, trans=None, ohne_leer=True, plot_peaks=False, txt_output=False ):
    ...
    if cali == None:
        cali = calibrate( False)

    file_name = files[ 1 if am else 0 ][file_number]
    if txt_output:
        print(file_name[:-4])
    directory = data_dir + roe_dir

    empty = Hist.read( directory + leer_names[ 0 ],
            "Leer",
            "Leer",
            calibration=cali )
    ...
    if am:
        ...
        scale = [20/50,35/50,1][file_number]
    print(scale)
    spectrum.hist.Scale(1/scale)
    empty.hist.Scale(1/scale)

    if ohne_leer:
        spectrum = Hist( spectrum.hist - empty.hist, "Spectrum", "Spectrum" )

    acc, height, fac, max_candidates = paras[ 0 ][ file_number ]
    ...
    if plot_peaks:
        plot_hist( spectrum, logy=True, save=True)
