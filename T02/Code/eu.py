...
(
        ...
        (peak_fits, peak_pos)
    ) = analyse_element(  data[2][0], data[2][1], rausch, cali, True, True, True )
...
gamma_peaks = []
scale *= np.sum(np.array(amplitudes))/np.sum(products['gamma'][2])
